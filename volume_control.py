...
class VolumeController:
    def __init__(self):
        # Get the speakers device wrapper from pycaw utils
        speakers = AudioUtilities.GetSpeakers()
        if speakers is None:
            raise RuntimeError("No audio device found")
        # AudioUtilities.GetSpeakers() returns an AudioDevice wrapper (not a COM IMMDevice).
        # Use its EndpointVolume property which returns an IAudioEndpointVolume instance.
...
if __name__ == "__main__":
    v = VolumeController()
    v.volume_to(0.3)
    print(f"Current Volume: {v.get_volume()*100:.2f}%")
    v.volume_up_a_bit()
    print(f"Volume after increase: {v.get_volume()*100:.2f}%")
